...
_NUMERIC_COLUMNS = list(range(5, 21))  # colonnes F..U numériques (devise = texte)
...
def _row_from_sim(sim: Simulation) -> list:
    return [
        ...
        float(sim.taux_fret if sim.taux_fret > 0 else sim.taux_change),
        float(sim.prix_dzd),
        float(sim.fret_dzd),
...
def export_simulations_excel(path: str | Path, sims: list[Simulation]) -> Path:
    ...
    try:
        ...
        widths = [12, 14, 16, 8, 8, 11, 15, 13, 11, 11, 17, 14, 18, 12, 17, 16, 14, 16, 16, 14, 17]
...
def simulation_summary_text(sim: Simulation) -> str:
    ...
    lines = [
        ...
        f"Taux achat : {decimal_to_str(sim.taux_change)} DA/{sim.devise}",
        f"Taux fret : {decimal_to_str(sim.taux_fret if sim.taux_fret > 0 else sim.taux_change)} DA/{sim.devise}",
        "",
